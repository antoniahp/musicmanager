...
This repo contains all the code of the website from https://musicmanager.com
...
1. Check that you have your .env.dev and .env.dev.db.  If for some reason these files are not generated, remember that you can find samples in the repository.
   ...
   DEBUG=1
   SECRET_KEY=Dd1AskVtrzbCioe
   DJANGO_ALLOWED_HOSTS=localhost 127.0.0.1 [::1]
   SQL_ENGINE=django.db.backends.postgresql
   SQL_DATABASE=musicmanager_dev
   SQL_USER=musicmanager
   SQL_PASSWORD=3uTCfbHNRXHFSnZ
   SQL_HOST=db
   SQL_PORT=5432
   DATABASE=postgres
   ...
   POSTGRES_PASSWORD=3uTCfbHNRXHFSnZ
   POSTGRES_DB=musicmanager_dev
    ```

2. Create docker volumes to save the data:
    ```sh
    $ docker volume create --name=musicmanager_postgres_data_dev
    $ docker volume create --name=musicmanager_grafana
    ```
...
* **Dockerhub:** You must create an [access token](https://docs.docker.com/docker-hub/access-tokens/) to download the Docker images from the production server. Then  you must create the following repositories in your Docker Hub account:
  ...
  * musicmanager-prometheus
  * musicmanager-grafana
 ...
 * **Environment files:** Once the server is created, you must copy the .env files in the production server inside **/opt/musicmanager** path.
    ```sh
    $ cat .env.prod
    $ cat .env.prod.db
...
It will automatically load the provisioning configurations and serve the dashboard in [http://musicmanager.com:3000](http://musicmanager.com:3000).
